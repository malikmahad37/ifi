...
const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    "catalog" | "inbox" | "contact" | "billing" | "blogs"
  >("catalog");
  ...
  const [viewMode, setViewMode] = useState<
    "list" | "edit-category" | "edit-product"
  >("list");
  ...
  const handleRemoveSeries = (
    ...
    if (window.confirm("Permanently delete this product series?")) {
      setTempCategories((prev) =>
        prev.map((c) =>
          c.id === catId
            ? { ...c, series: c.series.filter((s) => s.id !== seriesId) }
            : c,
        ),
      );
  ...
  const handleCategoryTranslate = async (
    id: string,
    field: "nameUrdu" | "descriptionUrdu",
  ...
  const handleSeriesTranslate = async (
    catId: string,
    seriesId: string,
    field: "nameUrdu" | "descriptionUrdu",
    textToTranslate: string,
  ...
  const updateInvoiceItem = (
    ...
    setInvoiceItems((prev) =>
      prev.map((item) => {
        ...
        // Auto-assign discount if product is selected
        if (field === "description") {
          const product = allProducts.find((p) => p.value === value);
          if (product && product.discountAmt) {
            updated.discountAmt = product.discountAmt;
          } else {
            updated.discountAmt = ""; // Clear if not found or no discount
          }
        }
  ...
  const allProducts = useMemo(() => {
    ...
    return (categories || []).flatMap((c) =>
      (c.series || []).slice(0, 100).map((s) => ({
        // Limit to prevent massive arrays
  ...
  // Auto-calculate discount when invoice items change based on quantities and their individual discounts
  useEffect(() => {
    const totalDiscount = invoiceItems.reduce((acc, item) => {
      const discount = parseFloat(item.discountAmt || "0") || 0;
      const qty = parseFloat(item.weight || "1") || 1; // if it's pieces, weight is mostly the qty input. We use weight field.
      return acc + discount * qty;
    }, 0);
    setInvoiceDiscount(totalDiscount);
  }, [invoiceItems]);
  ...
  const handleLoadInvoice = (inv: SavedInvoice) => {
    if (
      window.confirm(
        "Loading this invoice will overwrite your current unsaved changes. Continue?",
      )
    ) {
  ...
  const handleClearHistory = () => {
    ...
    if (
      window.confirm(
        "WARNING: Are you sure you want to delete ALL saved invoices? This cannot be undone.",
      )
    ) {
      setSavedInvoices([]);
    ...
    <>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-theme-base/10 pb-10 mb-10">
        ...
        >
          <div className="flex items-center gap-3 text-brand-lime mb-1 animate-fade-in">
            ...
            <p className="text-[9px] md:text-[10px] font-black tracking-[0.5em] uppercase opacity-60">
              Control Panel
            </p>
          </div>
          <h1 className="text-4xl md:text-6xl font-black text-theme-base tracking-tighter uppercase leading-none">
            COMMAND
          </h1>
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
            <span className="text-[10px] text-brand-lime uppercase tracking-widest opacity-60">
              Connected to Cloud
            </span>
          </div>
        </motion.div>
        ...
        <section className="space-y-6 animate-fade-in">
          {inquiries.length === 0 ? (
            <div className="glass-panel p-20 rounded-[2.5rem] text-center">
              ...
              <p className="text-theme-base/20 [.light-theme_&]:text-theme-base/60 text-xs font-black uppercase tracking-widest">
                No messages
              </p>
            </div>
          ) : (
              ...
              <div className="glass-panel p-6 md:p-8 rounded-[2rem] border border-theme-base/5 space-y-4">
                <div className="flex justify-between items-start">
                  <div className="space-y-1">
                    <p className="text-brand-lime font-black text-sm uppercase">
                      {iq.name}
                    </p>
                    <p className="text-theme-base/30 [.light-theme_&]:text-theme-base/70 text-[9px] uppercase font-black tracking-widest">
                      {iq.date}
                    </p>
                  </div>
                  <button
                ...
                <div className="flex justify-between items-center pt-2">
                  <p className="text-theme-base/40 font-mono text-xs">
                    {iq.phone}
                  </p>
                </div>
              </div>
        ...
        <div className="animate-fade-in">
          {viewMode === "list" ? (
            <div className="space-y-6">
              ...
              <div className="hidden md:block bg-theme-base/5 rounded-[2rem] overflow-hidden border border-theme-base/5 shadow-2xl">
                <table className="w-full text-left">
                  <thead>
                    <tr className="border-b border-theme-base/5 bg-theme-base/[0.01]">
                      <th className="px-8 py-5 text-[9px] font-black uppercase tracking-widest text-theme-base/30 [.light-theme_&]:text-theme-base/70">
                        PRODUCT
                      </th>
                      <th className="px-8 py-5 text-[9px] font-black uppercase tracking-widest text-theme-base/30 [.light-theme_&]:text-theme-base/70">
                        ITEMS
                      </th>
                      <th className="px-8 py-5 text-[9px] font-black uppercase tracking-widest text-theme-base/30 [.light-theme_&]:text-theme-base/70 text-right">
                        ACTIONS
                      </th>
                    </tr>
                  </thead>
                        ...
                        >
                          <td className="px-8 py-4">
                            <div className="flex items-center gap-4">
                              <img
                                src={cat.image}
                                className="w-12 h-12 rounded-lg object-cover border border-theme-base/10"
                              />
                              <div>
                                <p className="text-theme-base font-black text-sm uppercase">
                                  {cat.name}
                                </p>
                                <p className="urdu-text text-theme-base/50 text-sm leading-none">
                                  {cat.nameUrdu}
                                </p>
                              </div>
                            </div>
                          </td>
                          <td className="px-8 py-4 text-theme-base/20 [.light-theme_&]:text-theme-base/60 text-xs font-black">
                            {(cat?.series || []).length} items
                          </td>
                          <td className="px-8 py-4 text-right">
                            <div className="flex justify-end gap-2">
                              <button
                                ...
                                className="p-2.5 bg-theme-base/5 rounded-lg text-theme-base/20 [.light-theme_&]:text-theme-base/60 hover:text-theme-base"
                              >
                                <Edit2 className="w-3.5 h-3.5" />
                              </button>
                              <button
                                onClick={(e) => handleRemoveCategory(e, cat.id)}
                                className="p-2.5 bg-red-500/5 rounded-lg text-red-500/30 hover:text-red-500"
                              >
              ...
              <div className="md:hidden grid grid-cols-1 gap-4">
                {filteredCategories.map((cat) => (
                  <div
                    key={cat.id}
                    className="glass-panel p-5 rounded-2xl border border-white/5 flex gap-4"
                  >
                    <img
                      src={cat.image}
                    ...
                    <div className="flex-grow space-y-2">
                      <div>
                        ...
                        <p className="urdu-text text-brand-text/50 text-base leading-none">
                          {cat.nameUrdu}
                        </p>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-theme-base/20 [.light-theme_&]:text-theme-base/60 text-[10px] font-black uppercase">
                        ...
                        <div className="flex gap-2">
                          <button
                            ...
                            className="p-2 bg-theme-base/5 rounded-lg"
                          >
                            <Edit2 className="w-4 h-4 text-theme-base/40" />
                          </button>
                          <button
                            onClick={(e) => handleRemoveCategory(e, cat.id)}
                            className="p-2 bg-red-500/5 rounded-lg"
                          >
              ...
              <div className="flex justify-end pt-6">
                <button
                  onClick={saveAll}
                  className="flex items-center gap-3 bg-brand-lime text-black font-black uppercase tracking-widest px-10 py-4 rounded-xl shadow-2xl shadow-brand-lime/10 active:scale-95 transition-all text-[10px]"
                >
            ...
            <div className="space-y-8 animate-slide-up">
              <div className="flex items-center justify-between border-b border-theme-base/5 pb-6">
                <button
                  ...
                  className="flex items-center gap-2 text-theme-base/30 [.light-theme_&]:text-theme-base/70 text-[9px] font-black uppercase tracking-widest"
                >
                  <ArrowLeft className="w-4 h-4" /> BACK TO LIST
                </button>
                <button
                  onClick={saveAll}
                  className="bg-brand-lime text-black px-6 py-2.5 rounded-lg text-[9px] font-black uppercase tracking-widest"
                >
                  SAVE
                </button>
              </div>
                      ...
                      >
                        <div className="space-y-6">
                          <div className="space-y-3">
                            <label className="text-[9px] font-black text-theme-base/30 [.light-theme_&]:text-theme-base/70 uppercase tracking-widest">
                              Name (EN/UR)
                            </label>
                            <input
                              value={cat.name}
                              ...
                              className="w-full bg-input-bg border border-theme-base/10 rounded-xl px-5 py-4 text-theme-base font-black focus:border-brand-lime/50 transition-colors"
                            />
                            <input
                              dir="rtl"
                              value={cat.nameUrdu}
                              ...
                              className="w-full bg-input-bg border border-theme-base/10 rounded-xl px-5 py-4 text-brand-text urdu-text text-2xl focus:border-brand-lime/50 transition-colors"
                            />
                          </div>
                          <div className="space-y-3">
                            <label className="text-[9px] font-black text-theme-base/30 [.light-theme_&]:text-theme-base/70 uppercase tracking-widest">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                handleImageUpload(e, (base64) =>
                                  updateCategory(cat.id, "image", base64),
                                )
                              }
                          ...
                          <div className="space-y-3">
                            ...
                            <textarea
                              ...
                              className="w-full h-24 bg-input-bg border border-theme-base/10 rounded-xl px-5 py-4 text-theme-base text-xs font-bold resize-none focus:border-brand-lime/50 transition-colors"
                              placeholder="Category description..."
                            />
                            <textarea
                              dir="rtl"
                              value={cat.descriptionUrdu}
                              onChange={(e) =>
                                updateCategory(
                                  cat.id,
                                  "descriptionUrdu",
                              ...
                              className="w-full h-24 bg-input-bg border border-theme-base/10 rounded-xl px-5 py-4 text-brand-text urdu-text text-lg resize-none focus:border-brand-lime/50 transition-colors"
                              placeholder="زمرہ کی تفصیل..."
                            />
                          </div>
                        </div>
                        <div className="space-y-6">
                          <div className="space-y-2">
                            <div className="flex justify-between items-center mb-2">
                              ...
                              <button
                                onClick={() => addSeriesToGroup(cat.id)}
                                className="text-brand-text text-[9px] font-black uppercase hover:underline"
                              >
                                + Add Item
                              </button>
                            </div>
                            <div className="space-y-3">
                              {(cat?.series || []).map((s) => (
                                <div
                                  key={s.id}
                                  className="flex items-center justify-between p-4 bg-input-bg/60 rounded-xl border border-theme-base/5 hover:border-theme-base/10 transition-colors"
                                >
                                  <div className="flex items-center gap-3">
                                    ...
                                    />
                                    <div>
                                      <p className="text-theme-base font-bold text-xs">
                                        {s.name}
                                      </p>
                                      <p className="urdu-text text-brand-text/40 text-xs leading-none">
                                        {s.nameUrdu}
                                      </p>
                                    </div>
                                  </div>
                                  <div className="flex gap-1.5">
                                    <button
                                      ...
                                      className="p-2 text-theme-base/20 [.light-theme_&]:text-theme-base/60 hover:text-theme-base"
                                    >
                                      <Edit2 className="w-3.5 h-3.5" />
                                    </button>
                                    <button
                                      onClick={(e) =>
                                        handleRemoveSeries(e, cat.id, s.id)
                                      }
                                      className="p-2 text-red-500/20 hover:text-red-500"
                                    >
                                      <Trash2 className="w-3.5 h-3.5" />
                                    </button>
                                  </div>
                ...
                selectedProductId && (
                  <div className="glass-panel p-6 md:p-10 rounded-[2.5rem] space-y-10">
                    {tempCategories
                      ...
                      ?.series.filter((s) => s.id === selectedProductId)
                      .map((s) => (
                        <div
                          key={s.id}
                          className="grid grid-cols-1 lg:grid-cols-2 gap-10"
                          ...
                          <div className="space-y-6">
                            <div className="space-y-3">
                              ...
                              <input
                                ...
                                className="w-full bg-input-bg border border-theme-base/10 rounded-xl px-5 py-4 text-theme-base font-black"
                              />
                              <input
                                dir="rtl"
                                value={s.nameUrdu}
                                ...
                                className="w-full bg-input-bg border border-theme-base/10 rounded-xl px-5 py-4 text-brand-text urdu-text text-2xl"
                              />
                            </div>
                            <div className="space-y-3">
                              <label className="text-[9px] font-black text-theme-base/30 [.light-theme_&]:text-theme-base/70 uppercase tracking-widest">
                          ...
                          <div className="space-y-6">
                            <div className="space-y-2">
                              ...
                              <input
                                ...
                                className="w-full bg-input-bg border border-theme-base/10 rounded-xl p-4 text-theme-base text-xs font-mono"
                              />
                            </div>
                            <div className="space-y-2">
                              <label className="text-[9px] font-black text-theme-base/30 [.light-theme_&]:text-theme-base/70 uppercase tracking-widest">
                              ...
                              <input
                                type="number"
                                placeholder="Optional fixed discount amount"
                                value={s.discountAmt || ""}
                                onChange={(e) =>
                                  updateSeries(
                                    selectedCategoryId,
                                ...
                                className="w-full bg-input-bg border border-theme-base/10 rounded-xl p-4 text-brand-lime text-xs font-mono focus:border-brand-lime/50 transition-colors"
                              />
                            </div>
                            <div className="space-y-2">
                              <label className="text-[9px] font-black text-theme-base/30 [.light-theme_&]:text-theme-base/70 uppercase tracking-widest">
                              ...
                              <textarea
                                ...
                                className="w-full h-24 bg-input-bg border border-theme-base/10 rounded-xl p-4 urdu-text text-base text-brand-text/80 resize-none"
                              />
                            </div>
                            <div className="pt-4">
                              <button
                                ...
                                className="w-full py-4 bg-red-500/5 hover:bg-red-500 hover:text-white border border-red-500/10 text-red-500 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all"
                              >
                ...
                <div className="flex flex-col sm:flex-row flex-wrap gap-4 items-center w-full md:w-auto">
                  ...
                  <div className="flex gap-2 w-full sm:w-auto">
                    ...
                    <button
                      ...
                      className="flex-1 sm:flex-none justify-center bg-theme-base/5 hover:bg-theme-base/10 text-theme-base px-6 py-3.5 rounded-xl text-[10px] font-black uppercase flex items-center gap-2 border border-theme-base/10 transition-colors"
                    >
                  ...
                  <button
                    ...
                    className="w-full sm:w-auto bg-brand-lime text-black px-8 py-3.5 rounded-xl text-[10px] font-black uppercase shadow-lg shadow-brand-lime/10 flex items-center justify-center gap-2"
                  >
              ...
              <div className="space-y-8 animate-fade-in">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  ...
                  {savedInvoices.length > 0 && (
                    <button
                      ...
                      className="w-full sm:w-auto px-6 py-4 bg-red-500/5 hover:bg-red-500 hover:text-white text-red-500 border border-red-500/10 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2"
                    >
                ...
                {filteredHistory.length === 0 ? (
                  <motion.div
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="glass-panel p-20 rounded-[2.5rem] border border-theme-base/5 bg-theme-base/[0.01] text-center flex flex-col items-center justify-center pt-24 pb-24"
                  >
                    ...
                    <h3 className="2xl font-black uppercase tracking-widest text-theme-base/40 mb-2">
                      No Records Found
                    </h3>
                    <p className="text-theme-base/30 text-xs font-bold">
                      Try adjusting your search or generate a new invoice.
                    </p>
                  </motion.div>
                ) : (
                      ...
                      {filteredHistory.map((inv, idx) => (
                        ...
                        >
                          ...
                          <div className="flex justify-between items-start mb-6">
                            <div>
                              <div className="flex items-center gap-2 mb-1">
                                <span className="text-[9px] font-black tracking-widest uppercase text-brand-lime px-1.5 py-0.5 bg-brand-lime/10 rounded">
                                  {inv.id}
                                </span>
                                <span className="text-[9px] font-black tracking-widest uppercase text-theme-base/40">
                                  {new Date(inv.date).toLocaleDateString()}
                                </span>
                              </div>
                              <h3 className="text-lg font-black text-theme-base line-clamp-1">
                                {inv.customer.name || "Walk-in Customer"}
                              </h3>
                            </div>
                            <div className="bg-brand-lime/10 px-3 py-1.5 rounded-lg border border-brand-lime/20 flex-shrink-0">
                              <p className="text-brand-lime font-mono font-black text-xs">
                          ...
                          <div className="space-y-2 mb-8">
                            <p className="text-xs font-bold text-theme-base/60 flex items-center gap-2">
                              <span className="w-1.5 h-1.5 rounded-full bg-theme-base/20"></span>{" "}
                          ...
                          <div className="flex gap-2">
                            <button
                              ...
                              className="flex-1 bg-theme-base/5 hover:bg-theme-base hover:text-input-bg text-theme-base py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all text-center flex items-center justify-center gap-2"
                            >
                              OPEN
                            </button>
                            <button
                              onClick={() => handleQuickPrint(inv)}
                              className="flex-1 bg-brand-lime/10 hover:bg-brand-lime hover:text-black text-brand-lime py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all text-center flex items-center justify-center gap-2"
                            >
                              PRINT
                            </button>
                            <button
                              onClick={() => handleDeleteSavedInvoice(inv.id)}
                              className="w-12 flex-shrink-0 bg-red-500/5 text-red-500/60 hover:text-white hover:bg-red-500 rounded-xl transition-all flex items-center justify-center"
                            >
                              <Trash2 className="w-4 h-4" />
                            </button>
                          </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 space-y-6">
                  <div className="glass-panel p-6 rounded-3xl space-y-4 border border-theme-base/10 bg-theme-base/[0.03] shadow-sm">
                    <h3 className="text-[10px] font-black text-theme-base uppercase tracking-widest border-b border-theme-base/5 pb-2">
                      Customer Details
                    </h3>
                    <div className="space-y-3">
                      <input
                ...
                <div className="lg:col-span-2 space-y-6">
                  <div className="glass-panel p-6 rounded-3xl space-y-4 border border-theme-base/10 bg-theme-base/[0.03] shadow-sm">
                    <div className="flex justify-between items-center border-b border-theme-base/5 pb-2">
                      <h3 className="text-[10px] font-black text-theme-base uppercase tracking-widest">
                        Order Items
                      </h3>
                      <div className="text-[10px] font-black text-brand-text uppercase tracking-widest">
                        Total:{" "}
                        <span className="text-theme-base text-sm ml-2">
                    ...
                    <div className="space-y-3 mt-4 overflow-x-auto pb-2">
                      <div className="min-w-[700px]">
                        {invoiceItems.map((item, idx) => (
                          <div
                            key={item.id}
                            className="grid grid-cols-12 gap-2 items-center bg-theme-base/5 p-3 rounded-xl border border-theme-base/5 mb-2"
                          >
                            <div className="col-span-1 text-center text-[9px] text-theme-base/30 [.light-theme_&]:text-theme-base/70 font-bold">
                              {idx + 1}
                            </div>
                            ...
                            <div className="col-span-2 flex items-center justify-between pl-2">
                              <span className="text-xs font-black font-mono text-brand-lime [.light-theme_&]:text-brand-text">
                    ...
                    <button
                      ...
                      className="w-full py-3 border-2 border-dashed border-theme-base/10 rounded-xl text-[10px] font-black uppercase text-theme-base/30 [.light-theme_&]:text-theme-base/70 hover:text-brand-text hover:border-brand-lime/30 transition-all"
                    >
                      + Add Line Item
                    </button>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4 border-t border-theme-base/5">
                      <div className="space-y-4">
                        <label className="text-[9px] font-black text-theme-base uppercase tracking-widest">
                          Terms & Notes
                        </label>
                        <textarea
                          value={invoiceNotes}
                      ...
                      <div className="space-y-4">
                        <div className="flex justify-between items-center bg-white/5 p-3 rounded-xl border border-theme-base/5">
                          ...
                          <span className="text-sm font-mono text-theme-base">
                            Rs. {subTotal.toLocaleString()}
                          </span>
                        </div>
                        <div className="flex justify-between items-center gap-4 bg-white/5 p-3 rounded-xl border border-theme-base/5">
                          <label className="text-[9px] font-black text-theme-base uppercase tracking-widest whitespace-nowrap">
                            Discount (-)
                          </label>
                          <input
                            type="number"
                        ...
                        <div className="flex justify-between items-center gap-4 bg-white/5 p-3 rounded-xl border border-theme-base/5">
                          ...
                          <input
                            ...
                            value={invoiceTax}
                            onChange={(e) =>
                              setInvoiceTax(parseFloat(e.target.value) || 0)
                            }
                            className="w-24 bg-white/5 [.light-theme_&]:bg-white border border-theme-base/10 rounded-lg px-2 py-1 text-right text-xs font-mono"
                          />
                        </div>
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center border-t border-theme-base/10 pt-4 gap-2">
                          <label className="text-xs font-black text-brand-text uppercase tracking-widest">
                            Grand Total
                          </label>
                          <span className="text-xl sm:text-2xl font-black font-mono text-brand-lime">
                            Rs. {grandTotal.toLocaleString()}
                          </span>
                        </div>
                      </div>
      ...
      <div className="hidden print:block fixed inset-0 bg-white z-[9999] p-8 text-black">
        <div className="max-w-[210mm] mx-auto h-full flex flex-col justify-between">
          <div className="space-y-8">
            ...
            <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
              ...
              <p className="font-bold text-lg">
                {invoiceCustomer.name || "Walk-in Customer"}
              </p>
              {invoiceCustomer.phone && (
                <p className="text-sm text-gray-600">{invoiceCustomer.phone}</p>
              )}
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b-2 border-black">
                  <th className="py-2 text-xs font-black uppercase">
                    Description
                  </th>
                  <th className="py-2 text-xs font-black uppercase text-center">
                    Weight / Qty
                  </th>
                  <th className="py-2 text-xs font-black uppercase text-center">
                    Rate
                  ...
                  <th className="py-2 text-xs font-black uppercase text-right">
                    Amount
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                {invoiceItems.map((item, idx) => (
                  <tr key={idx}>
                    ...
                    <td className="py-3 text-sm text-center font-mono">
                      {item.weight}
                    </td>
                    <td className="py-3 text-sm text-center font-mono">
                      {item.rate}
                    ...
                    <td className="py-3 text-sm text-right font-mono font-bold">
              ...
              <tfoot>
                <tr className="border-t-2 border-black">
                  <td
                    colSpan={4}
                    className="py-2 text-right text-xs font-bold uppercase pr-8 text-gray-500"
                  >
                    Subtotal
                  </td>
                  <td className="py-2 text-right text-sm font-bold font-mono">
                    Rs. {subTotal.toLocaleString()}
                  </td>
                </tr>
                {invoiceDiscount > 0 && (
                  <tr>
                    ...
                    <td className="py-2 text-right text-sm font-bold font-mono">
                      - Rs. {invoiceDiscount.toLocaleString()}
                    </td>
                  </tr>
                )}
                <tr>
                ...
                <tr className="border-t border-black">
                  <td
                    colSpan={4}
                    className="py-4 text-right text-sm font-black uppercase pr-8"
                  >
                    Grand Total
                  </td>
                  <td className="py-4 text-right text-lg font-black font-mono">
                    Rs. {grandTotal.toLocaleString()}
                  </td>
                </tr>
              </tfoot>
          ...
          <p className="text-[10px] text-center text-gray-500 pt-8">
      ...
      {activeTab === "blogs" && (
        <section className="space-y-6 animate-fade-in">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 border-b border-theme-base/5 pb-6">
            <div>
              <h2 className="text-xl font-black text-theme-base uppercase tracking-widest">
                Blog Management
              </h2>
              <p className="text-xs text-theme-base/40">
                Manage your public news and articles
              </p>
            </div>
            <button
              onClick={() => {
                ...
                showToast(
                  "Created Draft Blog! Click edit to fill content.",
                  "success",
                );
              }}
          ...
          {(blogs || []).length === 0 ? (
            <div className="glass-panel p-20 rounded-[2.5rem] text-center">
              ...
              <p className="text-theme-base/20 text-xs font-black uppercase tracking-widest">
                No blog posts found
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-6">
              {(blogs || []).map((blog) => (
                <div
                  key={blog.id}
                  className="glass-panel p-6 rounded-3xl border border-theme-base/5 space-y-6"
                >
                    ...
                    <div className="flex items-center gap-4 flex-grow">
                      ...
                      <div className="flex-grow">
                        <input
                          value={blog.title}
                          onChange={(e) =>
                            onSaveBlog({ ...blog, title: e.target.value })
                          }
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            onSaveBlog({ ...blog, titleUrdu: e.target.value })
                          }
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-[10px] uppercase font-black tracking-widest text-theme-base/40">
                        English Content (HTML/Text)
                      </label>
                      <textarea
                        value={blog.content}
                        onChange={(e) =>
                          onSaveBlog({ ...blog, content: e.target.value })
                        }
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] uppercase font-black tracking-widest text-theme-base/40 block text-right">
                        Urdu Content
                      </label>
                      <textarea
                        dir="rtl"
                        ...
                        onChange={(e) =>
                          onSaveBlog({ ...blog, contentUrdu: e.target.value })
                        }
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="space-y-2">
                      ...
                      <input
                        value={blog.excerpt}
                        onChange={(e) =>
                          onSaveBlog({ ...blog, excerpt: e.target.value })
                        }
                        className="bg-theme-base/5 border border-theme-base/5 rounded-lg text-theme-base px-3 py-2 w-full text-xs"
                      />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] uppercase font-black tracking-widest text-theme-base/40 text-right block">
                        Excerpt (UR)
                      </label>
                      <input
                        dir="rtl"
                        value={blog.excerptUrdu}
                        onChange={(e) =>
                          onSaveBlog({ ...blog, excerptUrdu: e.target.value })
                        }
                        className="bg-theme-base/5 border border-theme-base/5 rounded-lg urdu-text text-brand-text px-3 py-2 w-full text-xs text-right"
                      />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] uppercase font-black tracking-widest text-theme-base/40">
                        Cover Image URL / base64
                      </label>
                      <input
                        type="file"
                        accept="image/*"
      ...
      {activeTab === "contact" && (
        <section className="space-y-10 animate-fade-in">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <button
              onClick={saveAll}
              className="bg-brand-lime text-black px-8 py-3.5 rounded-xl text-[10px] font-black uppercase shadow-lg shadow-brand-lime/10"
            >
              SAVE CHANGES
            </button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-theme-base/5 p-8 rounded-3xl space-y-4 border border-theme-base/5 shadow-xl">
              <label className="text-[9px] font-black text-theme-base uppercase tracking-widest">
                Business Phone
              </label>
              <input
                ...
                className="w-full bg-theme-base/10 border border-theme-base/10 rounded-xl px-5 py-4 text-theme-base font-bold outline-none focus:border-theme-base/30 transition-all"
              />
            </div>
            ...
            <div className="bg-theme-base/5 p-8 rounded-3xl space-y-4 md:col-span-2 border border-theme-base/5 shadow-xl">
              <label className="text-[9px] font-black text-theme-base uppercase tracking-widest">
            ...
            <div className="bg-theme-base/5 p-8 rounded-3xl space-y-4 md:col-span-2 border border-theme-base/5 shadow-xl">
              <label className="text-[9px] font-black text-brand-lime uppercase tracking-widest">
                Business Text (English)
              </label>
              <textarea
                value={tempContact.businessText || ""}
                onChange={(e) =>
                  setTempContact({ ...tempContact, businessText: e.target.value })
                }
                className="w-full h-32 bg-theme-base/10 border border-theme-base/10 rounded-xl px-5 py-4 text-theme-base font-bold outline-none focus:border-theme-base/30 transition-all resize-y"
                placeholder="Write the about us text here..."
              />
            </div>
